...
@section('admin')
    <div class="container" style="background-color: #fdc13d; padding: 30px 30px 30px 30px; margin: 20px 20px 20px 20px; border-radius: 20px">
        ...
        <a style="margin: 20px 20px 20px 20px;" type="submit" class="btn btn-primary" href="{{ route('admin.message.index') }}">back</a>
        <div style="margin: 20px 20px 20px 20px;">
            <a type="submit" class="btn btn-primary" href="{{ route('admin.message.edit', $sergl->id) }}">update</a>
        </div>
        <div style="margin: 20px 20px 20px 20px;">
            <form action="{{ route('admin.message.destroy', $sergl->id) }}" method="post">
                @csrf
                @method('delete')
